...
#define RADIUS_DEBUG(...) wpa_printf(MSG_DEBUG, "RADIUS SRV: " __VA_ARGS__)
#define RADIUS_ERROR(...) wpa_printf(MSG_ERROR, "RADIUS SRV: " __VA_ARGS__)
#define RADIUS_DUMP(...) wpa_hexdump(MSG_MSGDUMP, "RADIUS SRV: " __VA_ARGS__)
#define RADIUS_DUMP_ASCII(...)                                                 \
  wpa_hexdump_ascii(MSG_MSGDUMP, "RADIUS SRV: " __VA_ARGS__)
...
static int db_table_create_sim_provisioning(sqlite3 *db) {
  char *err = NULL;
  const char *sql = "CREATE TABLE sim_provisioning("
...
void srv_log(struct radius_session *sess, const char *fmt, ...) {
  ...
  if (sess->server->db) {
    ...
    sql = sqlite3_mprintf("INSERT INTO authlog"
                          ...
                          "'now'),%u,%Q,%Q,%Q)",
                          sess->sess_id, sess->nas_ip, sess->username, buf);
    if (sql) {
      if (sqlite3_exec(sess->server->db, sql, NULL, NULL, NULL) != SQLITE_OK) {
...
radius_server_get_client(struct radius_server_data *data, struct in_addr *addr,
  ...
  while (client) {
    ...
    if (ipv6) {
      ...
      for (i = 0; i < 16; i++) {
        if ((addr6->s6_addr[i] & client->mask6.s6_addr[i]) !=
            (client->addr6.s6_addr[i] & client->mask6.s6_addr[i])) {
          i = 17;
          break;
        }
      }
...
static void
radius_server_testing_options_tls(struct radius_session *sess, const char *tls,
  ...
  switch (test) {
    ...
    case 3:
      srv_log(sess,
              "TLS test - break ServerKeyExchange ServerParams Signature");
    ...
    case 7:
      srv_log(
          sess,
          "TLS test - RSA-DHE using a short 58-bit prime in long container");
...
radius_server_erp_find_key(struct radius_server_data *data,
                           const char *keyname) {
  struct eap_server_erp_key *erp;

  dl_list_for_each(erp, &data->erp_keys, struct eap_server_erp_key, list) {
...
radius_server_get_new_session(struct radius_server_data *data,
  ...
  if (radius_msg_get_attr_ptr(msg, RADIUS_ATTR_CALLING_STATION_ID, &id, &id_len,
    ...
    if (convert_ascii2mac(buf, sess->mac_addr) < 0)
      ...
      RADIUS_DEBUG("Calling-Station-Id: " MACSTR, MAC2STR(sess->mac_addr));
  }
  ...
  sess->eap = eap_server_sm_init(sess, &radius_server_eapol_cb, data->eap_cfg,
                                 &eap_sess);
  if (sess->eap == NULL) {
...
static void radius_srv_hs20_t_c_pending(struct radius_session *sess) {
  ...
  if (!sess->server->db || !sess->eap || is_zero_ether_addr(sess->mac_addr))
    return;
  ...
  sql = sqlite3_mprintf(
      "INSERT OR REPLACE INTO pending_tc (mac_addr,identity) VALUES (%Q,%Q)",
      addr, id_str);
  os_free(id_str);
  if (!sql)
    return;
...
static void radius_server_add_session(struct radius_session *sess) {
  ...
  os_snprintf(addr_txt, sizeof(addr_txt), MACSTR, MAC2STR(sess->mac_addr));

  sys_get_time(&now);
  sql = sqlite3_mprintf("INSERT OR REPLACE INTO "
                        "current_sessions(mac_addr,identity,start_time,nas,"
                        "hs20_t_c_filtering) VALUES (%Q,%Q,%d,%Q,%u)",
                        addr_txt, sess->username, now.sec, sess->nas_ip,
                        sess->t_c_filtering);
  if (sql) {
    if (sqlite3_exec(sess->server->db, sql, NULL, NULL, NULL) != SQLITE_OK) {
      RADIUS_ERROR(
          "Failed to add current_sessions entry into sqlite database: %s",
          sqlite3_errmsg(sess->server->db));
    }
    sqlite3_free(sql);
  }
...
static void db_update_last_msk(struct radius_session *sess, const char *msk) {
  ...
  sql = sqlite3_mprintf("UPDATE users SET last_msk=%Q WHERE identity=%Q", msk,
                        id_str);
  os_free(id_str);
  if (!sql)
    return;
...
static int radius_server_is_sim_method(struct radius_session *sess) {
  ...
  return name && (os_strcmp(name, "SIM") == 0 || os_strcmp(name, "AKA") == 0 ||
...
static int radius_server_hs20_missing_sim_pps(struct radius_msg *request) {
  ...
  buf = NULL;
  for (;;) {
    if (radius_msg_get_attr_ptr(request, RADIUS_ATTR_VENDOR_SPECIFIC, &buf,
                                &len, buf) < 0)
      return 0;
    if (len < 6)
      continue;
...
static int radius_server_sim_provisioning_session(struct radius_session *sess,
  ...
  os_snprintf(addr_txt, sizeof(addr_txt), MACSTR, MAC2STR(sess->mac_addr));
  wpa_snprintf_hex(hash_txt, sizeof(hash_txt), hash, HS20_MOBILE_ID_HASH_LEN);

  sys_get_time(&now);
  sql = sqlite3_mprintf("INSERT INTO "
                        "sim_provisioning(mobile_identifier_hash,imsi,mac_addr,"
                        "eap_method,timestamp) VALUES (%Q,%Q,%Q,%Q,%u)",
  ...
  if (sqlite3_exec(sess->server->db, sql, NULL, NULL, NULL) != SQLITE_OK) {
    RADIUS_ERROR(
        "Failed to add SIM provisioning entry into sqlite database: %s",
        sqlite3_errmsg(sess->server->db));
    res = -1;
  } else {
    res = 0;
...
static struct radius_msg *radius_server_encapsulate_eap(
    struct radius_server_data *data, struct radius_client *client,
    struct radius_session *sess, struct radius_msg *request) {
  (void)data;
  ...
  if (code == RADIUS_CODE_ACCESS_CHALLENGE &&
      !radius_msg_add_attr(msg, RADIUS_ATTR_STATE, (u8 *)&sess_id,
                           sizeof(sess_id))) {
  ...
  if (code == RADIUS_CODE_ACCESS_ACCEPT && sess->eap_if->eapKeyData) {
    ...
    if (len > 64)
      len = 64;
    len = wpa_snprintf_hex(buf, sizeof(buf), sess->eap_if->eapKeyData, len);
    buf[len] = '\0';
    ...
    if (!radius_msg_add_mppe_keys(
            msg, hdr->authenticator, (u8 *)client->shared_secret,
            client->shared_secret_len, sess->eap_if->eapKeyData + len, len,
  ...
  if (code == RADIUS_CODE_ACCESS_ACCEPT && sess->remediation &&
    ...
    if (!radius_msg_add_wfa(msg, RADIUS_VENDOR_ATTR_WFA_HS20_SUBSCR_REMEDIATION,
                            buf, 1 + url_len)) {
    ...
    RADIUS_DUMP("hotspot2dot0-mobile-identifier-hash", hash,
    ...
    *pos++ = data->subscr_remediation_method;
    ...
    pos += prefix_len;
    wpa_snprintf_hex((char *)pos, 2 * HS20_MOBILE_ID_HASH_LEN + 1, hash,
                     HS20_MOBILE_ID_HASH_LEN);
    RADIUS_DEBUG("HS 2.0 subscription remediation URL: %s", (char *)&buf[1]);
    if (!radius_msg_add_wfa(msg, RADIUS_VENDOR_ATTR_WFA_HS20_SUBSCR_REMEDIATION,
                            buf, 1 + url_len)) {
      RADIUS_DEBUG("Failed to add WFA-HS20-SubscrRem");
    }
  ...
  if (code == RADIUS_CODE_ACCESS_ACCEPT && sess->t_c_filtering) {
    ...
    if (!radius_msg_add_wfa(msg, RADIUS_VENDOR_ATTR_WFA_HS20_T_C_FILTERING, buf,
                            sizeof(buf))) {
    ...
    if (!radius_msg_add_wfa(msg, RADIUS_VENDOR_ATTR_WFA_HS20_T_C_URL,
  ...
  if (code == RADIUS_CODE_ACCESS_ACCEPT) {
    ...
    for (attr = sess->accept_attr; attr; attr = attr->next) {
      if (!radius_msg_add_attr(msg, attr->type, wpabuf_head(attr->val),
                               wpabuf_len(attr->val))) {
  ...
  if (code == RADIUS_CODE_ACCESS_REJECT) {
    if (radius_msg_add_attr_int32(msg, RADIUS_ATTR_WLAN_REASON_CODE, reason) <
        0) {
...
static struct radius_msg *radius_server_macacl(struct radius_server_data *data,
                                               struct radius_client *client,
                                               struct radius_session *sess,
  ...
  if (radius_msg_get_attr_ptr(request, RADIUS_ATTR_USER_PASSWORD, &pw, &pw_len,
    ...
    res = data->get_eap_user(data->conf_ctx, (u8 *)sess->username,
                             os_strlen(sess->username), 0, &tmp, NULL,
                             RADIUS_USER_NO_ATTR);
    if (res || !tmp.macacl || tmp.password == NULL) {
      ...
      res = radius_user_password_hide(
          request, tmp.password, tmp.password_len, (u8 *)client->shared_secret,
          client->shared_secret_len, buf, sizeof(buf));
      bin_clear_free(tmp.password, tmp.password_len);
...
static int radius_server_reject(struct radius_server_data *data,
  ...
  RADIUS_DEBUG("Reject invalid request from %s:%d", from_addr, from_port);
...
static void radius_server_hs20_t_c_check(struct radius_session *sess,
  ...
  buf = NULL;
  for (;;) {
    if (radius_msg_get_attr_ptr(msg, RADIUS_ATTR_VENDOR_SPECIFIC, &buf, &len,
                                buf) < 0)
      break;
    if (len < 6)
      continue;
  ...
  if (sess->t_c_timestamp != WPA_GET_BE32(timestamp)) {
    RADIUS_DEBUG("Last read T&C timestamp does not match HS20-Timestamp --> "
                 "require filtering");
...
static int radius_server_request(struct radius_server_data *data,
                                 struct radius_msg *msg, struct sockaddr *from,
                                 socklen_t fromlen,
                                 struct radius_client *client,
                                 const char *from_addr, int from_port,
  ...
  if (force_sess)
    ...
    res =
        radius_msg_get_attr(msg, RADIUS_ATTR_STATE, statebuf, sizeof(statebuf));
  ...
  if (sess->last_from_port == from_port &&
    ...
    if (sess->last_reply) {
      ...
      res = sendto(data->auth_sock, wpabuf_head(buf), wpabuf_len(buf), 0,
  ...
  if (eap == NULL) {
    RADIUS_DEBUG("No EAP-Message in RADIUS packet from %s", from_addr);
...
radius_server_receive_disconnect_resp(struct radius_server_data *data,
  ...
  if (hdr->identifier != client->pending_dac_disconnect_id) {
    RADIUS_DEBUG("Ignore unexpected Disconnect response with unexpected "
                 "identifier %u (expected %u)",
  ...
  RADIUS_DEBUG("Disconnect-%s received for " MACSTR, ack ? "ACK" : "NAK",
...
static void radius_server_receive_coa_resp(struct radius_server_data *data,
  ...
  if (hdr->identifier != client->pending_dac_coa_id) {
    RADIUS_DEBUG("Ignore unexpected CoA response with unexpected identifier %u "
                 "(expected %u)",
                 hdr->identifier, client->pending_dac_coa_id);
    radius_msg_free(msg);
    return;
  }

  if (radius_msg_verify(msg, (const u8 *)client->shared_secret,
                        client->shared_secret_len, client->pending_dac_coa_req,
                        0)) {
  ...
  RADIUS_DEBUG("CoA-%s received for " MACSTR, ack ? "ACK" : "NAK",
  ...
  if (ack) {
    sql = sqlite3_mprintf(
        "UPDATE current_sessions SET hs20_t_c_filtering=0, waiting_coa_ack=0, "
        "coa_ack_received=1 WHERE mac_addr=%Q",
        addrtxt);
  } else {
    sql = sqlite3_mprintf(
        "UPDATE current_sessions SET waiting_coa_ack=0 WHERE mac_addr=%Q",
        addrtxt);
  }
  if (!sql)
    return;
...
static void radius_server_receive_auth(int sock, void *eloop_ctx,
  ...
  fromlen = sizeof(from);
  len = recvfrom(sock, buf, RADIUS_MAX_MSG_LEN, 0, (struct sockaddr *)&from.ss,
                 &fromlen);
  if (len < 0) {
    wpa_printf(MSG_INFO, "recvfrom[radius_server]: %s", strerror(errno));
    goto fail;
  }

#ifdef CONFIG_IPV6
  if (data->ipv6) {
    if (inet_ntop(AF_INET6, &from.sin6.sin6_addr, abuf, sizeof(abuf)) == NULL)
      abuf[0] = '\0';
    from_port = ntohs(from.sin6.sin6_port);
    RADIUS_DEBUG("Received %d bytes from %s:%d", len, abuf, from_port);

    client = radius_server_get_client(
        data, (struct in_addr *)&from.sin6.sin6_addr, 1);
  }
#endif /* CONFIG_IPV6 */
  ...
  if (radius_msg_get_hdr(msg)->code != RADIUS_CODE_ACCESS_REQUEST) {
    RADIUS_DEBUG("Unexpected RADIUS code %d", radius_msg_get_hdr(msg)->code);
  ...
  if (radius_server_request(data, msg, (struct sockaddr *)&from, fromlen,
                            client, abuf, from_port, NULL) == -2)
...
static void radius_server_receive_acct(int sock, void *eloop_ctx,
  ...
  radius_msg_finish_acct_resp(resp, (u8 *)client->shared_secret,
                              client->shared_secret_len, hdr->authenticator);
...
static int radius_server_open_socket6(int port) {
  ...
  if (s < 0) {
    wpa_printf(MSG_INFO, "RADIUS: socket[IPv6]: %s", strerror(errno));
    return -1;
  }
  ...
  while (fgets(buf, buf_size, f)) {
    ...
    if (*pos == '/') {
      ...
      *pos++ = '\0';
      ...
      if ((pos == end) || (mask < 0 || mask > (ipv6 ? 128 : 32))) {
        failed = 1;
        break;
      }
      pos = end;
    ...
    if (ipv6 && inet_pton(AF_INET6, buf, &addr6) <= 0) {
      ...
      os_memcpy(addr6.s6_addr + 12, (char *)&addr.s_addr, 4);
    }
#endif /* CONFIG_IPV6 */
...
struct radius_server_data *radius_server_init(struct radius_server_conf *conf) {
  ...
  if (conf->subscr_remediation_url) {
    data->subscr_remediation_url = os_strdup(conf->subscr_remediation_url);
  ...
  if (conf->sqlite_file) {
    if (sqlite3_open(conf->sqlite_file, &data->db)) {
      RADIUS_ERROR("Could not open SQLite file '%s'", conf->sqlite_file);
      goto fail;
    }
  }
#endif /* CONFIG_SQLITE */
  ...
  data->clients = radius_server_read_clients(conf->client_file, conf->ipv6);
  ...
  if (conf->acct_port) {
#ifdef CONFIG_IPV6
    if (conf->ipv6)
      data->acct_sock = radius_server_open_socket6(conf->acct_port);
    else
#endif /* CONFIG_IPV6 */
    ...
    if (eloop_register_read_sock(data->eloop, data->acct_sock,
                                 radius_server_receive_acct, data, NULL))
      goto fail;
  } else {
    data->acct_sock = -1;
  }
...
void radius_server_deinit(struct radius_server_data *data) {
  ...
  if (data->acct_sock >= 0) {
    eloop_unregister_read_sock(data->eloop, data->acct_sock);
...
int radius_server_get_mib(struct radius_server_data *data, char *buf,
  ...
  ret = os_snprintf(
      ...
      data->counters.access_requests, data->counters.invalid_requests,
      data->counters.dup_access_requests, data->counters.access_accepts,
      data->counters.access_rejects, data->counters.access_challenges,
      data->counters.malformed_access_requests,
      data->counters.bad_authenticators, data->counters.packets_dropped,
      data->counters.unknown_types, data->counters.acct_requests,
      data->counters.invalid_acct_requests, data->counters.acct_responses,
      data->counters.malformed_acct_requests,
      data->counters.acct_bad_authenticators,
      data->counters.unknown_acct_types);
  if (os_snprintf_error(end - pos, ret)) {
  ...
  for (cli = data->clients, idx = 0; cli; cli = cli->next, idx++) {
...
#ifdef CONFIG_IPV6
    if (data->ipv6) {
      if (inet_ntop(AF_INET6, &cli->addr6, abuf, sizeof(abuf)) == NULL)
        abuf[0] = '\0';
      if (inet_ntop(AF_INET6, &cli->mask6, mbuf, sizeof(mbuf)) == NULL)
        mbuf[0] = '\0';
    }
#endif /* CONFIG_IPV6 */
    if (!data->ipv6) {
    ...
    ret = os_snprintf(
        ...
        "radiusAccServTotalBadAuthenticators=%u\n"
        "radiusAccServTotalUnknownTypes=%u\n",
        idx, abuf, mbuf, cli->counters.access_requests,
        cli->counters.dup_access_requests, cli->counters.access_accepts,
        cli->counters.access_rejects, cli->counters.access_challenges,
        cli->counters.malformed_access_requests,
        cli->counters.bad_authenticators, cli->counters.packets_dropped,
        cli->counters.unknown_types, cli->counters.acct_requests,
        cli->counters.invalid_acct_requests, cli->counters.acct_responses,
...
static int radius_server_get_eap_user(void *ctx, const u8 *identity,
  ...
  ret = data->get_eap_user(data->conf_ctx, identity, identity_len, phase2, user,
                           NULL, RADIUS_USER_VLAN_ATTR);
  if (ret == 0 && user) {
...
static int radius_server_erp_add_key(void *ctx,
                                     struct eap_server_erp_key *erp) {
  struct radius_session *sess = ctx;
  struct radius_server_data *data = sess->server;
...
void radius_server_eap_pending_cb(struct radius_server_data *data, void *ctx) {
  ...
  if (radius_server_request(data, msg, (struct sockaddr *)&sess->last_from,
                            sess->last_fromlen, cli, sess->last_from_addr,
...
static int get_db_session_fields(void *ctx, int argc, char *argv[],
                                 char *col[]) {
...
int radius_server_dac_request(struct radius_server_data *data,
                              const char *req) {
  (void)data;
  (void)req;
#ifdef CONFIG_SQLITE
  ...
  sqlite3_free(sql);
  if (res != SQLITE_OK) {
    RADIUS_ERROR("Failed to find matching current_sessions entry from sqlite "
                 "database: %s",
                 sqlite3_errmsg(data->db));
  ...
  os_snprintf(addrtxt, sizeof(addrtxt), RADIUS_802_1X_ADDR_FORMAT,
              MAC2STR(addr));
  if (!radius_msg_add_attr(msg, RADIUS_ATTR_CALLING_STATION_ID, (u8 *)addrtxt,
                           os_strlen(addrtxt))) {
  ...
  if (!disconnect && t_c_clear) {
    ...
    if (!radius_msg_add_wfa(msg, RADIUS_VENDOR_ATTR_WFA_HS20_T_C_FILTERING, val,
                            sizeof(val))) {
      RADIUS_DEBUG("Failed to add WFA-HS20-T-C-Filtering");
      radius_msg_free(msg);
      return -1;
  ...
  sys_get_time(&now);
  if (!radius_msg_add_attr_int32(msg, RADIUS_ATTR_EVENT_TIMESTAMP, now.sec)) {
  ...
  if (sendto(data->auth_sock, wpabuf_head(buf), wpabuf_len(buf), 0,
    ...
    RADIUS_ERROR("Failed to send packet - sendto: %s", strerror(errno));
    radius_msg_free(msg);
    return -1;
  }
