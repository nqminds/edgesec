		      ...
		      size_t secret_len)
{
	...
	if (wpabuf_len(msg->buf) > 0xffff) {
		log_trace("RADIUS: Too long message (%lu)", (unsigned long) wpabuf_len(msg->buf));
		return -1;
	}
	return 0;
}
			    ...
			    size_t secret_len)
{
	...
	if (wpabuf_len(msg->buf) > 0xffff) {
		log_trace("RADIUS: Too long messages (%lu)", (unsigned long) wpabuf_len(msg->buf));
	}
}
			      ...
			      int require_message_authenticator)
{
	...
	for (i = 0; i < msg->attr_used; i++) {
		...
		if (tmp->type == RADIUS_ATTR_MESSAGE_AUTHENTICATOR) {
			if (attr != NULL) {
				log_trace("Multiple Message-Authenticator attributes in RADIUS message");
				return 1;
			}
			attr = tmp;
		}
					    ...
					    const uint8_t *data, size_t data_len)
{
	...
	if (data_len > RADIUS_MAX_ATTR_LEN) {
		log_trace("radius_msg_add_attr: too long attribute (%lu bytes)", (unsigned long) data_len);
		return NULL;
	}
...
struct radius_msg * radius_msg_parse(const uint8_t *data, size_t len)
{
	...
	if (msg_len < len) {
		log_trace("RADIUS: Ignored %lu extra bytes after RADIUS message", (unsigned long) len - msg_len);
	}
...
int radius_msg_add_wfa(struct radius_msg *msg, uint8_t subtype, const uint8_t *data,
		       size_t len)
{
	struct radius_attr_hdr *attr;
	uint8_t *buf, *pos;
