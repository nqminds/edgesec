...
get_vlan_attribute(uint16_t vlan_id, struct hostapd_radius_attr **last) {
  char id_str[5];
  struct hostapd_radius_attr *attr = NULL, *attr_medium_type = NULL,
                             *attr_id = NULL;
...
struct hostapd_radius_attr *
get_tunnel_pass_attribute(const uint8_t *req_authenticator,
...
int save_user_attribute(struct radius_context *context, const u8 *identity,
                        size_t identity_len, struct eap_user *user,
                        struct radius_msg *msg, enum RADIUS_USER_ATTR user_attr,
                        struct identity_info *iinfo) {

  if (user_attr == RADIUS_USER_VLAN_ATTR ||
      user_attr == RADIUS_USER_VLANPASS_ATTR) {
    struct hostapd_radius_attr *last_attr = NULL;
    struct hostapd_radius_attr *vlan_attr =
        get_vlan_attribute(iinfo->vlanid, &last_attr);
    ...
    if (msg != NULL && iinfo->id_pass_len &&
      ...
      struct hostapd_radius_attr *pass_attr = get_tunnel_pass_attribute(
          hdr->authenticator, (const uint8_t *)context->rconf->radius_secret,
          os_strlen(context->rconf->radius_secret), iinfo->id_pass,
          iinfo->id_pass_len);
      if (pass_attr == NULL) {
    ...
    if (put_attr_mapper(&context->attr_mapper, identity, identity_len,
                        vlan_attr) < 0) {
      log_error("put_attr_mapper fail");
      free_attr(vlan_attr);
      return -1;
    }
    if (get_attr_mapper(&context->attr_mapper, identity, identity_len,
                        &user->accept_attr) < 0) {
...
int radius_get_eap_user(void *ctx, const u8 *identity, size_t identity_len,
  ...
  if (identity_len && identity != NULL) {
    ...
    if (context->get_identity_ac_fn != NULL) {
      ...
      if ((iinfo = context->get_identity_ac_fn(identity, identity_len,
                                               context->ctx_cb)) == NULL) {
        ...
        if (iinfo->access == IDENTITY_ACCESS_DENY) {
          ...
          if (save_user_attribute(context, identity, identity_len, user, msg,
                                  user_attr, iinfo) < 0) {
...
int generate_client_conf(struct radius_conf *rconf) {
  ...
  fprintf(fp, "%s/%d %s\n", rconf->radius_client_ip, rconf->radius_client_mask,
          rconf->radius_secret);
...
void *init_eap_tls(struct radius_conf *rconf) {
  ...
  log_trace("Loading EAP server certificate file %s",
            rconf->eap_server_cert_path);
  tparams.client_cert = rconf->eap_server_cert_path;

  log_trace("Loading EAP server private key file %s",
            rconf->eap_server_key_path);
