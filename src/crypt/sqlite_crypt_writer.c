...
int open_sqlite_crypt_db(char *db_path, sqlite3** sql)
{
  ...
  *sql = db;
  ...
  rc = check_table_exists(db, CRYPT_TABLE_NAME);

  if (rc == 0) {
    log_debug("table doesn't exist creating...");
    if (execute_sqlite_query(db, CRYPT_CREATE_TABLE) < 0) {
...
int save_sqlite_crypt_entry(sqlite3 *db, char *key, char *value)
{
  ...
  if (sqlite3_prepare_v2(db, CRYPT_INSERT_INTO, -1, &res, 0) != SQLITE_OK) {
