...
static void sha256_init_buf_state(struct buffer_state *state, const void *input, size_t len)
{
	state->p = input;
	state->len = len;
	state->total_len = len;
...
static inline const uint8_t *sha256_calc_chunk(uint8_t chunk[CHUNK_SIZE], struct buffer_state *state)
{
			...
			for (j = 0; j < 16; j++) {
				if (i == 0) {
					...
					const uint32_t s0 = sha256_right_rot(w[(j + 1) & 0xf], 7) ^
							    sha256_right_rot(w[(j + 1) & 0xf], 18) ^ (w[(j + 1) & 0xf] >> 3);
					const uint32_t s1 = sha256_right_rot(w[(j + 14) & 0xf], 17) ^
							    sha256_right_rot(w[(j + 14) & 0xf], 19) ^
							    (w[(j + 14) & 0xf] >> 10);
					w[j] = w[j] + s0 + w[(j + 9) & 0xf] + s1;
				}
				const uint32_t s1 = sha256_right_rot(ah[4], 6) ^ sha256_right_rot(ah[4], 11) ^ sha256_right_rot(ah[4], 25);
				const uint32_t ch = (ah[4] & ah[5]) ^ (~ah[4] & ah[6]);
				const uint32_t temp1 = ah[7] + s1 + ch + k[i << 4 | j] + w[j];
				const uint32_t s0 = sha256_right_rot(ah[0], 2) ^ sha256_right_rot(ah[0], 13) ^ sha256_right_rot(ah[0], 22);
