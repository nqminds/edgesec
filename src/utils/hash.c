...
static inline uint32_t sha256_right_rot(uint32_t value, unsigned int count) {
  /*
...
static void sha256_init_buf_state(struct buffer_state *state, const void *input,
                                  size_t len) {
  state->p = input;
  state->len = len;
  state->total_len = len;
...
static inline const uint8_t *sha256_calc_chunk(uint8_t chunk[CHUNK_SIZE],
                                               struct buffer_state *state) {
  if (state->total_len_delivered) {
    return NULL;
  }

  /* For whole chunks, there is no need to copy data, we just return the
   * original chunk. */
...
/*
 * Limitations:
 * - Since input is a pointer, the data to hash must be directly accessible to
 * the processor, which could be a problem for large data sizes.
...
void sha256_hash(uint8_t hash[SHA256_HASH_LEN], const void *input, size_t len) {
   ...
   * the message schedule array w[i], 0
   * <= i <= 63.
   *
  ...
  while ((p = sha256_calc_chunk(chunk, &state))) {
    uint32_t ah[8];
     ...
     * initial values in w[0..63] don't matter, so many implementations zero
     * them here) copy chunk into first 16 words w[0..15] of the message
     * schedule array
     */
    uint32_t w[16];
    ...
    for (i = 0; i < 4; i++) {
      for (j = 0; j < 16; j++) {
        if (i == 0) {
          ...
          const uint32_t s0 = sha256_right_rot(w[(j + 1) & 0xf], 7) ^
                              sha256_right_rot(w[(j + 1) & 0xf], 18) ^
                              (w[(j + 1) & 0xf] >> 3);
          const uint32_t s1 = sha256_right_rot(w[(j + 14) & 0xf], 17) ^
                              sha256_right_rot(w[(j + 14) & 0xf], 19) ^
                              (w[(j + 14) & 0xf] >> 10);
          w[j] = w[j] + s0 + w[(j + 9) & 0xf] + s1;
        }
        const uint32_t s1 = sha256_right_rot(ah[4], 6) ^
                            sha256_right_rot(ah[4], 11) ^
                            sha256_right_rot(ah[4], 25);
        const uint32_t ch = (ah[4] & ah[5]) ^ (~ah[4] & ah[6]);
        const uint32_t temp1 = ah[7] + s1 + ch + k[i << 4 | j] + w[j];
        const uint32_t s0 = sha256_right_rot(ah[0], 2) ^
                            sha256_right_rot(ah[0], 13) ^
                            sha256_right_rot(ah[0], 22);
        const uint32_t maj =
            (ah[0] & ah[1]) ^ (ah[0] & ah[2]) ^ (ah[1] & ah[2]);
