  ...
  while ((p = (char **)utarray_next(column_arr, p)) != NULL) {
    if (strlen(*p)) {
      switch (state) {
        ...
        case 6:
          // in column
          sys_strlcpy(row.in, *p, IF_NAMESIZE);
          state = 7;
          break;
        case 7:
          // out column
          sys_strlcpy(row.out, *p, IF_NAMESIZE);
          state = 8;
          break;
        case 8:
          // source column
          sys_strlcpy(row.source, *p, OS_INET_ADDRSTRLEN);
          state = 9;
          break;
        case 9:
          // destination column
          sys_strlcpy(row.destination, *p, OS_INET_ADDRSTRLEN);
          state = 10;
          break;
      }
    }
...
struct iptables_context *iptables_init(const char *path, UT_array *ifinfo_array,
  ...
  sys_strlcpy(ctx->iptables_path, path, MAX_OS_PATH_LEN);
