...
static void test_save_sqlite_fingerprint_row(void **state)
{
  ...
  struct fingerprint_row row = {.mac = mac, .protocol = protocol, .fingerprint = fingerprint, 
...
static void test_get_sqlite_fingerprint_rows(void **state)
{
  ...
  char *op = "<=", *op1 = ">";
  UT_array *rows;
  struct fingerprint_row in = {.mac = mac, .protocol = protocol, .fingerprint = fingerprint, 
                                .timestamp = timestamp, .query = query}, *p = NULL;
