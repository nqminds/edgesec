...
struct radius_client_data {
   ...
   */
  void (*interim_error_cb)(const uint8_t *addr, void *ctx);
...
static int radius_change_server(struct radius_client_data *radius,
                                ...
                                struct hostapd_radius_server *oserv, int sock,
                                int sock6, int auth);
...
int radius_client_register(
  ...
  newh =
      os_realloc_array(*handlers, *num + 1, sizeof(struct radius_rx_handler));
  if (newh == NULL)
    return -1;
...
static int radius_client_retransmit(struct radius_client_data *radius,
  ...
  if (entry->msg_type == RADIUS_ACCT_INTERIM) {
    log_trace(
        ...
        "%02x:%02x:%02x:%02x:%02x:%02x - drop message and request a new update",
        MAC2STR(entry->addr));
    if (radius->interim_error_cb)
      radius->interim_error_cb(entry->addr, radius->interim_error_cb_ctx);
    return 1;
  }

  if (s < 0) {
    log_trace("RADIUS: No valid socket for retransmission");
    return 1;
  }
  ...
  if (entry->msg_type == RADIUS_ACCT &&
    ...
    log_trace("RADIUS: Updated Acct-Delay-Time to %u for retransmission",
              delay_time);
  ...
  if (entry->accu_attempts >
      RADIUS_CLIENT_MAX_FAILOVER * RADIUS_CLIENT_NUM_FAILOVER * num_servers) {
    log_trace("RADIUS: Removing un-ACKed message due to too many failed "
              "retransmit attempts");
    return 1;
  }
  ...
  sys_get_reltime(&entry->last_attempt);
...
static void radius_client_timer(void *eloop_ctx, void *timeout_ctx) {
  ...
  while (entry) {
    ...
    if (prev_num_msgs != radius->num_msgs) {
      log_trace("RADIUS: Message removed from queue - restart from beginning");
      entry = radius->msgs;
      prev = NULL;
      continue;
    }
...
static void radius_client_auth_failover(struct radius_client_data *radius) {
  ...
  log_trace("No response from Authentication server %s:%d - failover",
            hostapd_ip_txt(&old->addr, abuf, sizeof(abuf)), old->port);
  ...
  radius_change_server(radius, next, old, radius->auth_serv_sock,
...
static void radius_client_acct_failover(struct radius_client_data *radius) {
  ...
  log_trace("No response from Accounting server %s:%d - failover",
            hostapd_ip_txt(&old->addr, abuf, sizeof(abuf)), old->port);
  ...
  radius_change_server(radius, next, old, radius->acct_serv_sock,
...
static void radius_client_list_add(struct radius_client_data *radius,
  ...
  if (radius->num_msgs >= RADIUS_CLIENT_MAX_ENTRIES) {
    log_trace("RADIUS: Removing the oldest un-ACKed packet due to retransmit "
              "list limits");
    prev = NULL;
    while (entry->next) {
      prev = entry;
...
int radius_client_send(struct radius_client_data *radius,
  ...
  if (msg_type == RADIUS_ACCT || msg_type == RADIUS_ACCT_INTERIM) {
    ...
    if (conf->acct_server == NULL || radius->acct_sock < 0 ||
      ...
      log_trace("No accounting server configured");
      return -1;
    }
    ...
    if (conf->auth_server == NULL || radius->auth_sock < 0 ||
      ...
      log_trace("No authentication server configured");
      return -1;
    }
  ...
  log_trace(
      "Sending RADIUS(%02x:%02x:%02x:%02x:%02x:%02x) message to %s server",
      MAC2STR(addr), name);
  if (conf->msg_dumps)
    radius_msg_dump(msg);
...
static void radius_client_receive(int sock, void *eloop_ctx, void *sock_ctx) {
  ...
  if (len == sizeof(buf)) {
    log_trace(
        "RADIUS: Possibly too long UDP frame for our buffer - dropping it");
    return;
  }

  msg = radius_msg_parse(buf, len);
  if (msg == NULL) {
    log_trace("RADIUS: Parsing incoming frame failed");
    rconf->malformed_responses++;
    return;
  }
  hdr = radius_msg_get_hdr(msg);

  log_trace("Received RADIUS message");
  if (conf->msg_dumps)
    radius_msg_dump(msg);
  ...
  while (req) {
    ...
    if ((req->msg_type == msg_type ||
         (req->msg_type == RADIUS_ACCT_INTERIM && msg_type == RADIUS_ACCT)) &&
        radius_msg_get_hdr(req->msg)->identifier == hdr->identifier)
      break;
  ...
  for (i = 0; i < num_handlers; i++) {
    ...
    res = handlers[i].handler(msg, req->msg, req->shared_secret,
                              req->shared_secret_len, handlers[i].data);
    switch (res) {
  ...
  if (invalid_authenticator)
    ...
    rconf->unknown_types++;
  log_trace(
      "No RADIUS RX handler found (type=%d code=%d id=%d)%s - dropping packet",
...
uint8_t radius_client_get_id(struct radius_client_data *radius) {
  ...
  while (entry) {
    if (radius_msg_get_hdr(entry->msg)->identifier == id) {
      log_trace("Removing pending RADIUS message, since its id (%d) is reused",
                id);
      if (prev)
        prev->next = entry->next;
      else
...
static void radius_client_update_acct_msgs(struct radius_client_data *radius,
  ...
  for (entry = radius->msgs; entry; entry = entry->next) {
    if (entry->msg_type == RADIUS_ACCT) {
      entry->shared_secret = shared_secret;
      entry->shared_secret_len = shared_secret_len;
      radius_msg_finish_acct(entry->msg, shared_secret, shared_secret_len);
    }
  }
}
...
static int radius_change_server(struct radius_client_data *radius,
  ...
  log_trace("%s server %s:%d", auth ? "Authentication" : "Accounting",
            hostapd_ip_txt(&nserv->addr, abuf, sizeof(abuf)), nserv->port);
  ...
  switch (nserv->addr.af) {
    ...
    case AF_INET6:
      ...
      os_memcpy(&serv6.sin6_addr, &nserv->addr.u.v6, sizeof(struct in6_addr));
  ...
  switch (nserv->addr.af) {
    case AF_INET:
      claddrlen = sizeof(claddr);
      if (getsockname(sel_sock, (struct sockaddr *)&claddr, &claddrlen) == 0) {
        log_trace("RADIUS local address: %s:%u", inet_ntoa(claddr.sin_addr),
    ...
    case AF_INET6: {
      claddrlen = sizeof(claddr6);
      if (getsockname(sel_sock, (struct sockaddr *)&claddr6, &claddrlen) == 0) {
        wpa_printf(MSG_DEBUG, "RADIUS local address: %s:%u",
                   inet_ntop(AF_INET6, &claddr6.sin6_addr, abuf, sizeof(abuf)),
  ...
  if (radius->acct_sock >= 0 && conf->acct_servers &&
    ...
    if (radius_change_server(radius, conf->acct_server, oserv,
      ...
      radius_change_server(radius, oserv, conf->acct_server,
                           radius->acct_serv_sock, radius->acct_serv_sock6, 0);
    }
  }
...
static int radius_client_init_auth(struct radius_client_data *radius) {
  ...
  radius_change_server(radius, conf->auth_server, NULL, radius->auth_serv_sock,
                       radius->auth_serv_sock6, 1);
...
static int radius_client_init_acct(struct radius_client_data *radius) {
  ...
  radius_change_server(radius, conf->acct_server, NULL, radius->acct_serv_sock,
                       radius->acct_serv_sock6, 0);
...
radius_client_init(struct eloop_data *eloop, void *ctx,
  ...
  radius->auth_serv_sock = radius->acct_serv_sock = radius->auth_serv_sock6 =
      radius->acct_serv_sock6 = radius->auth_sock = radius->acct_sock = -1;
...
static int radius_client_dump_auth_server(char *buf, size_t buflen,
  ...
  return snprintf(buf, buflen,
                  ...
                  serv->index, hostapd_ip_txt(&serv->addr, abuf, sizeof(abuf)),
                  serv->port, serv->round_trip_time, serv->requests,
                  serv->retransmissions, serv->access_accepts,
                  serv->access_rejects, serv->access_challenges,
                  serv->malformed_responses, serv->bad_authenticators, pending,
                  serv->timeouts, serv->unknown_types, serv->packets_dropped);
}
...
static int radius_client_dump_acct_server(char *buf, size_t buflen,
  ...
  if (cli) {
    for (msg = cli->msgs; msg; msg = msg->next) {
      if (msg->msg_type == RADIUS_ACCT || msg->msg_type == RADIUS_ACCT_INTERIM)
        pending++;
    }
  }
  ...
  return snprintf(buf, buflen,
                  ...
                  serv->index, hostapd_ip_txt(&serv->addr, abuf, sizeof(abuf)),
                  serv->port, serv->round_trip_time, serv->requests,
                  serv->retransmissions, serv->responses,
                  serv->malformed_responses, serv->bad_authenticators, pending,
                  serv->timeouts, serv->unknown_types, serv->packets_dropped);
}
...
int radius_client_get_mib(struct radius_client_data *radius, char *buf,
  ...
  if (conf->acct_servers) {
    for (i = 0; i < conf->num_acct_servers; i++) {
      ...
      count += radius_client_dump_acct_server(buf + count, buflen - count, serv,
                                              serv == conf->acct_server ? radius
                                                                        : NULL);
    }
  }
