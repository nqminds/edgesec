...
void radius_client_set_interim_error_cb(struct radius_client_data *radius,
					void (*cb)(const uint8_t *addr, void *ctx),
					void *ctx)
{
				    ...
				    os_time_t now)
{
	...
	if (entry->msg_type == RADIUS_ACCT_INTERIM) {
		log_trace("RADIUS: Failed to transmit interim accounting update to %02x:%02x:%02x:%02x:%02x:%02x - drop message and request a new update", MAC2STR(entry->addr));
	...
	if (entry->msg_type == RADIUS_ACCT &&
		...
		log_trace("RADIUS: Updated Acct-Delay-Time to %u for retransmission", delay_time);
	...
	if (entry->accu_attempts > RADIUS_CLIENT_MAX_FAILOVER *
		...
		log_trace("RADIUS: Removing un-ACKed message due to too many failed retransmit attempts");
		return 1;
	}

	entry->attempts++;
	entry->accu_attempts++;
	log_trace("Resending RADIUS message (id=%d)", radius_msg_get_hdr(entry->msg)->identifier);
...
static void radius_client_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (radius->msgs) {
		...
		log_trace("Next RADIUS client retransmit in %ld seconds", (long int) (first - now.sec));
	}
}
				   ...
				   size_t shared_secret_len, const uint8_t *addr)
{
	...
	if (radius->num_msgs >= RADIUS_CLIENT_MAX_ENTRIES) {
		log_trace("RADIUS: Removing the oldest un-ACKed packet due to retransmit list limits");
		prev = NULL;
		while (entry->next) {
			prev = entry;
			entry = entry->next;
		       ...
		       const uint8_t *addr)
{
	...
	log_trace("Sending RADIUS(%02x:%02x:%02x:%02x:%02x:%02x) message to %s server", MAC2STR(addr), name);
	if (conf->msg_dumps)
		radius_msg_dump(msg);
...
static void radius_client_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	if (len == sizeof(buf)) {
		log_trace("RADIUS: Possibly too long UDP frame for our buffer - dropping it");
		return;
	}
	...
	if (req == NULL) {
		log_trace("No matching RADIUS request found (type=%d id=%d) - dropping packet", msg_type, hdr->identifier);
		goto fail;
	}
	...
	log_trace("Received RADIUS packet matched with a pending request, round trip time %d.%02d sec",
	...
	if (invalid_authenticator)
		...
		rconf->unknown_types++;
	log_trace("No RADIUS RX handler found (type=%d code=%d id=%d)%s - dropping packet",
...
uint8_t radius_client_get_id(struct radius_client_data *radius)
{
	...
	while (entry) {
		if (radius_msg_get_hdr(entry->msg)->identifier == id) {
			log_trace("Removing pending RADIUS message, since its id (%d) is reused", id);
			if (prev)
				prev->next = entry->next;
			else
				radius->msgs = entry->next;
		     ...
		     int sock, int sock6, int auth)
{
	...
	if (sel_sock < 0) {
		log_trace("RADIUS: No server socket available (af=%d sock=%d sock6=%d auth=%d",
	...
	case AF_INET:
		...
		if (getsockname(sel_sock, (struct sockaddr *) &claddr,
				&claddrlen) == 0) {
			log_trace("RADIUS local address: %s:%u",
...
static int radius_client_init_auth(struct radius_client_data *radius)
{
	...
	if (radius->auth_serv_sock >= 0 &&
	    eloop_register_read_sock(radius->auth_serv_sock,
				     ...
				     (void *) RADIUS_AUTH)) {
		log_trace("RADIUS: Could not register read socket for authentication server");
		radius_close_auth_sockets(radius);
		return -1;
	}
			      ...
			      const uint8_t *addr)
{
	...
	while (entry) {
		if (entry->msg_type == RADIUS_AUTH &&
			...
			log_trace("Removing pending RADIUS authentication message for removed client");
